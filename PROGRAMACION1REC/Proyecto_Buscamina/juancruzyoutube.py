...
def crear_bombas_random(numero, primer_click, dificultad):
    ...
    while bombas > 0:
        random_row = random.randint(0, filas - 1)
        random_col = random.randint(0, columnas - 1)

        # Evitar colocar bombas en la región 3x3 del primer clic
        if (
        ...
        # Colocar bomba si la celda está vacía
...
def color_numero(element,color):
    global tamano_cuadricula
    valor_casilla = pygame.font.render(str(calidades_casilla[element]['valor']),True,color)
            ...
            if not primera_celda_pulsada:
                ...
                if keys[2]:
                    if not calidades_casilla[element]['pulsada'] == True and not calidades_casilla[element]['bandera'] == True and not bombas_restantes == 0:
                        valor_casilla = pygame.font.render('?',True,(0,0,0))
                        ventana =pygame.ventana.blit(valor_casilla,(calidades_casilla[element]['posicion_xy'].x + 10,calidades_casilla[element]['posicion_xy'].y + 5))
                        pygame.display.update()
...
def mostrar_bombas(numero,casillas):
    global bombas_restantes, ventana
    ...
    pygame.ventana.blit(start_text, (x,y))
    pygame.display.update()
